...
def test_long_is_blocked_when_all_directional_indicators_oppose():
    result = evaluate_indicator_filter(
        snapshot(ema20=99, ema50=100, rsi14=45, macd=-1, macd_signal=0),
        "LONG",
    )
...
def test_short_is_blocked_when_all_directional_indicators_oppose():
    result = evaluate_indicator_filter(
        snapshot(ema20=101, ema50=100, rsi14=55, macd=1, macd_signal=0),
        "SHORT",
    )
    assert result.allowed is False
    assert result.state == "OPPOSED"
...
def test_mixed_indicator_context_does_not_destroy_price_action_setup():
    result = evaluate_indicator_filter(
        snapshot(ema20=101, ema50=100, rsi14=45, macd=1, macd_signal=0),
        "LONG",
    )
...
def test_unwarmed_indicators_fail_open_to_price_action():
    result = evaluate_indicator_filter(
        snapshot(ema20=None, ema50=None, rsi14=None, macd=None, macd_signal=None),
        "LONG",
    )
    assert result.allowed is True
