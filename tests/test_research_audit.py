...
def _evidence(stability=True, robustness=True):
    return ValidationEvidence(
        evidence_sha256="evidence", timeframe="1h", dataset_sha256="dataset", config_sha256="config",
        walk_forward={"fold_count": 1},
        ml={"fold_count": 1, "trained_fold_count": 1, "threshold": 0.55, "horizon_bars": 3,
            "folds": [{"fold_index": 1, "train_last_signal_index": 6, "train_last_label_end_index": 9,
                        "test_first_signal_index": 12, "test_last_signal_index": 18,
                        "train_feature_sha256": "a" * 64, "test_feature_sha256": "b" * 64}]},
        regime=({"regime": "BULLISH", "samples": 5, "positive": 3},),
        stability=({"sample_count": 10, "feature_count": 2, "baseline_accuracy": 0.8, "importance": []} if stability else None),
        robustness=({"case_count": 2, "scenario_names": ("base", "stress"), "net_r_range": 1.0,
                     "expectancy_r_range": 0.5, "cases": []} if robustness else None),
    )


def _fold(*, fold_index=1, test_start=10, test_end=20, train_samples=10, train_positive=5,
          test_labeled_samples=4, model_trained=True, train_last_signal_index=6,
          train_last_label_end_index=9, test_first_signal_index=12, test_last_signal_index=18,
          train_feature_sha256="a" * 64, test_feature_sha256="b" * 64):
    return SimpleNamespace(
        fold_index=fold_index, test_start=test_start, test_end=test_end,
        train_samples=train_samples, train_positive=train_positive, test_labeled_samples=test_labeled_samples,
        model_trained=model_trained, train_last_signal_index=train_last_signal_index,
        train_last_label_end_index=train_last_label_end_index, test_first_signal_index=test_first_signal_index,
        test_last_signal_index=test_last_signal_index, train_feature_sha256=train_feature_sha256,
...
def _ml(*, folds=None, history_bars=10, test_bars=10, step_bars=10):
    if folds is None:
        folds = (_fold(),)
    return SimpleNamespace(timeframe="1h", history_bars=history_bars, test_bars=test_bars,
                           step_bars=step_bars, horizon_bars=3, fold_count=len(folds),
                           trained_fold_count=sum(fold.model_trained for fold in folds), folds=tuple(folds))
...
def test_audit_rejects_invalid_ml_threshold():
    evidence = _evidence(); evidence.ml["threshold"] = 1.0
    report = audit_validation_evidence(evidence, _ml())
...
def test_audit_rejects_overlapping_oos_windows():
    ...
    evidence = _evidence(); evidence.ml["fold_count"] = 2; evidence.ml["trained_fold_count"] = 2
    evidence.ml["folds"].append({**evidence.ml["folds"][0], "fold_index": 2,
                                 "test_first_signal_index": 19, "test_last_signal_index": 28})
...
def test_audit_rejects_incorrect_fold_spacing():
    ...
    evidence = _evidence(); evidence.ml["fold_count"] = 2; evidence.ml["trained_fold_count"] = 2
    evidence.ml["folds"].append({**evidence.ml["folds"][0], "fold_index": 2,
                                 "test_first_signal_index": 25, "test_last_signal_index": 34})
    report = audit_validation_evidence(evidence, _ml(folds=folds))
    assert not report.passed
...
def test_audit_rejects_invalid_feature_fingerprint():
    fold = _fold(train_feature_sha256="not-a-sha")
    report = audit_validation_evidence(_evidence(), _ml(folds=(fold,)))
    assert not report.passed
    assert any("invalid train_feature_sha256" in item for item in report.findings)
