...
def indicator_context(signal):
    values = signal.indicators
    if values is None:
        return {"values": None, "trend": "UNAVAILABLE", "momentum": "UNAVAILABLE", "macd_momentum": "UNAVAILABLE", "trend_strength": "UNAVAILABLE", "direction": None, "confirmations": 0, "confirmation_state": "NEUTRAL"}
    trend = "UNAVAILABLE"
    ...
    if direction is None:
        state = "NEUTRAL"
    return {"values": indicator_payload(signal), "trend": trend, "momentum": momentum, "macd_momentum": macd_momentum, "trend_strength": trend_strength, "direction": direction, "confirmations": confirmations, "confirmation_state": state}
...
def signal_payload(evaluation):
    ...
    return {
        ...
        "forecast": {"confidence": forecast_result.confidence if forecast_result else None, "horizons": [] if forecast_result is None else [{"direction": h.direction, "up_probability": h.up_probability, "flat_probability": h.flat_probability, "down_probability": h.down_probability, "expected_return": h.expected_return, "expected_close": h.expected_close} for h in forecast_result.horizons]},
        "supervisor": None if evaluation.supervisor is None else {"action": evaluation.supervisor.action, "allowed": evaluation.supervisor.allowed, "reasons": list(evaluation.supervisor.reasons)},
    }
...
def main() -> None:
    ...
    output = []
    for case in cases:
        bars = [LiveBar(time=datetime.fromisoformat(row["time"].replace("Z", "+00:00")), open=float(row["open"]), high=float(row["high"]), low=float(row["low"]), close=float(row["close"])) for row in case["candles"]]
        monitor = RealtimeMonitor(FixtureFeed(bars), case["symbol"], case["timeframe"], lookback=len(bars))
