...
broker. It models the minimum execution semantics needed to exercise ARIA's
order-state, idempotency, reconciliation, and recovery layers:

- full, partial, rejected, and timeout-after-accept outcomes
- deterministic fills and configurable fill price
- client-order idempotency
- connection loss / recovery
- order and position snapshots for reconciliation

The simulator never sends network traffic and cannot place a real order.
"""
...
class PaperBrokerSimulator:
    ...
    def submit(self, request: PaperOrderRequest) -> PaperOrderSnapshot:
        """Submit one request with deterministic broker-like semantics.

        A timeout-after-accept deliberately records the order as FILLED but
        raises ``TimeoutError`` to model the dangerous ambiguous-response case:
        the client must reconcile before attempting any retry.
        """
    ...
    def _validate_request(request: PaperOrderRequest) -> None:
        ...
        if request.submitted_at.tzinfo != timezone.utc:
