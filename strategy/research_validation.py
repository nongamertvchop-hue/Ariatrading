...
def _walk_forward_payload(controlled: ControlledResearchResult) -> dict[str, Any]:
    ...
    return {
        ...
        "folds": [
            {"fold_index": fold.fold_index, "history_start": fold.history_start,
...
def _ml_payload(result: MLWalkForwardResult) -> dict[str, Any]:
    return {
        ...
        "folds": [
            {"fold_index": fold.fold_index, "train_samples": fold.train_samples,
             "train_positive": fold.train_positive, "test_labeled_samples": fold.test_labeled_samples,
             ...
             "train_feature_sha256": fold.train_feature_sha256,
             "test_feature_sha256": fold.test_feature_sha256}
            for fold in result.folds
        ],
    }
...
def _stability_payload(report: MLStabilityReport) -> dict[str, Any]:
    return {"sample_count": report.sample_count, "feature_count": report.feature_count,
...
def _robustness_payload(report: RobustnessReport) -> dict[str, Any]:
    return {"case_count": report.case_count, "scenario_names": report.scenario_names,
            "net_r_range": report.net_r_range, "expectancy_r_range": report.expectancy_r_range,
            "cases": [{"name": case.name, "metrics": asdict(case.metrics)} for case in report.cases]}


def build_validation_evidence(
    controlled: ControlledResearchResult, ml_result: MLWalkForwardResult, *,
    regime_stats: tuple[RegimeStats, ...] | list[RegimeStats] = (),
    stability: MLStabilityReport | None = None, robustness: RobustnessReport | None = None,
) -> ValidationEvidence:
    ...
    payload = {"timeframe": ml_result.timeframe, "dataset_sha256": control.dataset.sha256,
               "config_sha256": control.config_sha256, "walk_forward": _walk_forward_payload(controlled),
               "ml": _ml_payload(ml_result), "regime": [asdict(item) for item in regime_stats],
    ...
    return ValidationEvidence(evidence_sha256=evidence_sha256, timeframe=ml_result.timeframe,
                              dataset_sha256=control.dataset.sha256, config_sha256=control.config_sha256,
                              walk_forward=payload["walk_forward"], ml=payload["ml"],
                              regime=tuple(payload["regime"]), stability=payload["stability"],
