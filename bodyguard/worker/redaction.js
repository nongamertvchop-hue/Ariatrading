...
function redactString(value) { let result=String(value); for(const pattern of SECRET_VALUE_RES) result=result.replace(pattern,REDACTED); return result; }
export function sanitizeForBoundary(value,{maxDepth=8,maxItems=200}={}){
  const visit=(item,depth)=>{
    ...
    if(Array.isArray(item))return item.slice(0,maxItems).map(child=>visit(child,depth+1));
    if(typeof item==="object"){
      const result=Object.create(null);let index=0;
      for(const[key,child]of Object.entries(item)){
        if(index>=maxItems){result["[TRUNCATED]"]=REDACTED;break;}
        result[String(key)]=SENSITIVE_KEY_RE.test(String(key))?REDACTED:visit(child,depth+1);index++;
      }
      return result;
    }
