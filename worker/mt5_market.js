...
function validateCandle(raw, timeframe) {
  ...
  if (time % TIMEFRAME_SECONDS[timeframe] !== 0) throw new Error("candle time is not aligned to timeframe");
...
function validatePayload(payload) {
  ...
  const normalized=candles.map(c=>validateCandle(c,timeframe));
  ...
  return {symbol,timeframe,candles:normalized,liveCandle,receivedAt,price:Number(payload?.price)};
}
...
export class Mt5MarketStore {
  ...
  async ingest(request) {
    try {
      ...
      await this.state.storage.put(key(payload.symbol,payload.timeframe),{candles:candles.slice(-MAX_CANDLES),live_candle:payload.liveCandle,price,received_at:payload.receivedAt});
