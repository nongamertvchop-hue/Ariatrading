...
import { LONG, SHORT, WAIT, NO_BREAKOUT, TIMEFRAME_CONFIG, BadRequest, adaptiveZoneTolerance, evaluateSequence, analyzeMarketStructure, findSupportZones, findResistanceZones, scoreSetup, validateCandle } from "./signal_parity.js";
...
function stopReference(zone, direction, history, timeframe) { const config = TIMEFRAME_CONFIG[timeframe]; const sample = history.slice(-config.lookback); const range = sample.reduce((sum, candle) => sum + (candle.high - candle.low), 0) / Math.min(history.length, config.lookback); const buffer = Math.max(config.minZoneDistance * 0.5, range * config.confirmationMultiplier); return direction === LONG ? zone.low - buffer : zone.high + buffer; }
...
export function evaluateRealtimeSignalParity(rawCandles, timeframe, minForecastConfidence = 0.45) {
  ...
  const longCandidates = supports.map((zone) => { const result = evaluateSequence(candles, zone, timeframe, LONG, 3); return { direction: LONG, zone, result, score: result.action === LONG ? scoreSetup(LONG, zone.touches, structure.bias, result.breakoutState, 20) : null }; });
  const shortCandidates = resistances.map((zone) => { const result = evaluateSequence(candles, zone, timeframe, SHORT, 3); return { direction: SHORT, zone, result, score: result.action === SHORT ? scoreSetup(SHORT, zone.touches, structure.bias, result.breakoutState, 20) : null }; });
...
export async function handleSignalParityV2(request, env) {
  ...
  if (payload.source !== "mt5" || !Array.isArray(payload.candles) || !/^[0-9a-f]{64}$/.test(payload.market_fingerprint || "")) return json({ error: "mt5_market_contract_rejected", message: "MT5 market contract or snapshot fingerprint rejected", source: "unavailable", execution: "NONE" }, 503);
