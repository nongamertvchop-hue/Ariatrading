(() => {
  ...
  function persist() {
    try {
      ...
      // UI preference only; persistence failure is non-fatal.
    }
  }
  ...
  function ensureOverlay() {
    let canvas = document.getElementById('indicator-overlay');
    const chart = document.getElementById('chart');
    const wrap = document.getElementById('chartwrap');
    if (!canvas || !chart || !wrap) return null;
  ...
  function viewport(candles) {
    ...
    return { data: candles.slice(start, end), start, end };
  }
  ...
  function draw() {
    ...
    if (!state || !chart || !wrap || !overlay || !state.candles?.length) return;

    refreshCache(state.candles);
    const data = viewport(state.candles).data;
  ...
  function loop() {
    const state = window.S;
    if (state?.signal?.indicators) renderSnapshot(state.signal.indicators);
    draw();
