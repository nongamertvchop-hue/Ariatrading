...
function normalizeRuntimePayload(payload, symbol, timeframe) {
  if (!payload || payload.source !== "mt5" || payload.execution !== "NONE") throw new Error("runtime market contract rejected");
  if (typeof payload.market_fingerprint !== "string" || !/^[0-9a-f]{64}$/.test(payload.market_fingerprint)) throw new Error("runtime market fingerprint rejected");
  if (!Array.isArray(payload.candles) || payload.candles.length < 21) throw new Error("runtime returned too few completed candles");
  const candles = payload.candles.map(validateCandle);
  for (let i = 1; i < candles.length; i += 1) if (candles[i].time <= candles[i - 1].time) throw new Error("runtime candles are not strictly chronological");
  const live = validateCandle(payload.live_candle);
  if (live.time <= candles.at(-1).time) throw new Error("runtime forming candle must be newer than completed history");
...
export async function onRequestGet(context) {
  ...
  if (!TIMEFRAMES.has(timeframe)) return json({ error: "bad_request", message: `unsupported timeframe: ${timeframe}` }, 400);
  const runtimeUrl = String(context.env?.MT5_RUNTIME_API_URL || "").trim().replace(/\/$/, "");
  if (!runtimeUrl) return json({ error: "live_data_unavailable", message: "MT5_RUNTIME_API_URL is not configured", symbol, timeframe, source: "unavailable", execution: "NONE" }, 503);
  const token = String(context.env?.RUNTIME_API_TOKEN || "").trim();
  if (!token) return json({ error: "live_data_unavailable", message: "RUNTIME_API_TOKEN is not configured", symbol, timeframe, source: "unavailable", execution: "NONE" }, 503);
  ...
  try {
    const response = await fetch(target, { signal: controller.signal, headers: { accept: "application/json", authorization: `Bearer ${token}`, "cache-control": "no-cache" } });
    let payload;
    ...
    return json({ error: "live_data_unavailable", message: error?.name === "AbortError" ? "MT5 runtime request timed out" : (error?.message || "MT5 runtime unavailable"), symbol, timeframe, source: "unavailable", execution: "NONE" }, 503);
