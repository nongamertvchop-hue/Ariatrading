...
(function(){'use strict';
  ...
  function setLiveStrategyState(state,payload){state.signal={...(state.signal||{}),symbol:payload.symbol,timeframe:payload.timeframe,source:payload.source,price:state.price,live_candle:payload.live_candle,signal:'WAIT',direction:'WAIT',state:'LIVE_DATA_ONLY',reason:'MT5 broker market data connected; strategy evaluation remains paper-only.',structure_bias:'—',breakout_state:'—',score:null,zone:null,entry_reference:null,stop_reference:null};}
  async function poll(){const state=window.S;if(!state?.symbol||!state?.tf)return;try{const payload=await liveMarket(state.symbol,state.tf),completed=normalizeCandles(payload.candles),liveCandle=normalizeCandle(payload.live_candle);if(!completed.length||!liveCandle)throw new Error('MT5 returned incomplete candle history');const displayCandles=completed.slice(),last=displayCandles.at(-1);if(!last||liveCandle.time>last.time)displayCandles.push(liveCandle);else if(liveCandle.time===last.time)displayCandles[displayCandles.length-1]=liveCandle;state.candles=displayCandles;state.price=Number(payload.price);setLiveStrategyState(state,payload);renderLiveQuote(payload,liveCandle);status(`MT5 LIVE · ${new Date().toLocaleTimeString()}`,true);if(typeof window.renderSignal==='function')window.renderSignal(state.signal);if(typeof window.draw==='function')window.draw();}catch(error){status(`MT5 DATA UNAVAILABLE · ${error?.message||'broker feed error'}`,false);}}
