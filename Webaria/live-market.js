/* Webaria live market bridge. MT5 broker feed is the chart source of truth. */
(function(){'use strict';
  ...
  async function evaluateStrategy(symbol,timeframe,candles){
    const response=await originalFetch('/api/strategy',{method:'POST',cache:'no-store',headers:{'content-type':'application/json'},body:JSON.stringify({symbol,timeframe,candles})});
    let payload;
  ...
  async function poll(){
    ...
    try{
      const payload=await liveMarket(symbol,timeframe);
      if(generation!==pollGeneration||window.S?.symbol!==symbol||window.S?.tf!==timeframe)return;
      const completed=normalizeCandles(payload.candles),liveCandle=normalizeCandle(payload.live_candle);
      if(!completed.length||!liveCandle)throw new Error('MT5 returned incomplete candle history');
      const displayCandles=completed.slice(),last=displayCandles.at(-1);
      if(!last||liveCandle.time>last.time)displayCandles.push(liveCandle);else if(liveCandle.time===last.time)displayCandles[displayCandles.length-1]=liveCandle;
      state.candles=displayCandles;
      state.price=Number(payload.price);
      renderLiveQuote(payload,liveCandle);

      const strategyCandles=completed.slice(-100).map(c=>({open:c.open,high:c.high,low:c.low,close:c.close,datetime:new Date(c.time).toISOString()}));
      const strategy=await evaluateStrategy(symbol,timeframe,strategyCandles);
      if(generation!==pollGeneration||window.S?.symbol!==symbol||window.S?.tf!==timeframe)return;
      state.signal={...strategy,live_candle:payload.live_candle,price:state.price,source:'mt5'};
      status(`MT5 LIVE · STRATEGY LIVE · ${new Date().toLocaleTimeString()}`,true);
      if(typeof window.renderSignal==='function')window.renderSignal(state.signal);
      if(typeof window.WebariaObservedSignalJournalUI?.observe==='function')window.WebariaObservedSignalJournalUI.observe(state.signal,state.symbol,state.tf);
      if(typeof window.draw==='function')window.draw();
    }catch(error){
      if(generation===pollGeneration&&window.S?.symbol===symbol&&window.S?.tf===timeframe)status(`MT5 DATA UNAVAILABLE · ${error?.message||'broker feed error'}`,false);
    }
  }
