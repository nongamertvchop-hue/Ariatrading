...
(function () {
  ...
  function loadLocalCache() {
    try {
      ...
      return {
        balance: finite(saved.balance, START_BALANCE),
  ...
  function cacheLocal() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (_) { /* cache is non-authoritative */ }
  }
  ...
  function currentPrice() {
    const p = Number($('quote')?.textContent?.trim().replace(/,/g, ''));
    return Number.isFinite(p) && p > 0 ? p : null;
  }

  function pnl(position, price) {
    if (!position || !Number.isFinite(price)) return 0;
    return (price - Number(position.entry)) * (position.side === 'LONG' ? 1 : -1) * Number(position.quantity);
  }

  function setNote(message) {
    const node = $('paperNote');
    if (node) node.textContent = message || 'Simulation only · durable paper runtime · orders never reach a broker';
  }

  function render() {
    const position = state.position;
    const price = currentPrice();
    const unrealized = position && position.symbol === currentSymbol() ? pnl(position, price) : 0;
    const balance = finite(state.balance, START_BALANCE);
    $('balance').textContent = balance.toFixed(2);
    $('equity').textContent = (balance + unrealized).toFixed(2);
    $('pos').textContent = position ? `${position.side} ${finite(position.quantity, 0)}` : 'FLAT';
    $('entry').textContent = position ? formatPrice(position.entry) : '—';
    $('pnl').textContent = `${unrealized >= 0 ? '+' : ''}${unrealized.toFixed(2)}`;
    $('close').disabled = !position || position.symbol !== currentSymbol() || !Number.isFinite(price);
    $('buy').disabled = !!position;
    $('sell').disabled = !!position;
    $('paperMetrics').innerHTML = buildMetrics();
    renderHistory();
    const note = document.querySelector('.paper-contract');
    if (note) note.textContent = authoritative ? 'contract aria.paper-runtime.v1 · durable state authoritative · execution NONE' : 'contract aria.paper-runtime.v1 · syncing durable state… · execution NONE';
  }
  ...
  function buildMetrics() {
    ...
    let balance = START_BALANCE, peak = START_BALANCE, maxDd = 0;
    ...
    return `<div class="metric"><span>Trades</span><b>${trades.length}</b></div>
      <div class="metric"><span>Win rate</span><b>${winRate.toFixed(1)}%</b></div>
      <div class="metric"><span>Net P/L</span><b>${format(trades.reduce((s,t)=>s+Number(t.pnl),0))}</b></div>
      <div class="metric"><span>Profit factor</span><b>${pf===Infinity?'∞':format(pf)}</b></div>
      <div class="metric"><span>Max drawdown</span><b>${format(maxDd)}</b></div>`;
  }
  ...
  function renderHistory() {
    ...
    node.innerHTML = rows.length ? rows.map(t => {
      const result = Number(t.pnl);
      const cls = result > 0 ? 'up' : result < 0 ? 'down' : '';
      return `<div class="paper-trade"><span>${escapeHtml(t.side)} · ${escapeHtml(t.symbol)}</span><b class="${cls}">${result >= 0 ? '+' : ''}${result.toFixed(2)}</b><small>${escapeHtml(t.reason || '')}</small></div>`;
    }).join('') : '<div class="paper-history-empty">No paper trades yet.</div>';
  }
  ...
  function normalizeBars(payload) {
    ...
    return payload.candles.map(raw => ({time: raw?.datetime ?? raw?.time, open:Number(raw?.open), high:Number(raw?.high), low:Number(raw?.low), close:Number(raw?.close)}))
      .filter(bar => Number.isFinite(timeValue(bar.time)) && [bar.open,bar.high,bar.low,bar.close].every(Number.isFinite) && bar.high >= bar.low)
  ...
  async function open(side) {
    if (!authoritative) { await readDurable(); if (!authoritative) return; }
    if (state.position) { setNote('Only one paper position is allowed at a time.'); return; }
    const symbol=currentSymbol(), timeframe=currentTimeframe(), entry=currentPrice(), quantity=Number($('qty')?.value), rawSl=$('sl')?.value?.trim(), rawTp=$('tp')?.value?.trim();
    const sl=rawSl?Number(rawSl):null, tp=rawTp?Number(rawTp):null;
    if (!Number.isFinite(entry)) return setNote('Cannot open: no valid market quote.');
    if (!Number.isFinite(quantity)||quantity<=0) return setNote('Rejected: quantity must be finite and > 0.');
    try { window.WebariaPaperEngine?.validateStops(side, entry, sl, tp); } catch (error) { return setNote(`Rejected: ${error.message}`); }
    let bars; try { bars=await getCompletedBars(symbol,timeframe); } catch(error) { return setNote(`Cannot open: ${error.message}`); }
    const latest=bars.at(-1); if(!latest) return setNote('Cannot open: no completed candle.');
    const next={...state,position:{symbol,timeframe,side,quantity,entry,sl:Number.isFinite(sl)?sl:null,tp:Number.isFinite(tp)?tp:null,opened_at:new Date().toISOString(),entry_bar_time:latest.time,entry_bar_close:latest.close,last_checked_bar_time:latest.time,execution_model:'completed-bar'}};
  ...
  async function resetPaperAccount() {
    if (state.position) return setNote('Reset rejected while a position is open. Close it first.');
    await writeDurable({balance:START_BALANCE,position:null,history:[]},'Paper account reset · durable state');
  }

  async function monitorPosition() {
    if (monitorBusy || !state.position || !authoritative) return;
    monitorBusy=true;
    try {
      const position=state.position, bars=await getCompletedBars(position.symbol,position.timeframe), anchor=timeValue(position.entry_bar_time), lastChecked=timeValue(position.last_checked_bar_time);
    ...
    } catch(error) { setNote(`Completed-bar monitor error: ${error.message}`); } finally { monitorBusy=false; }
  }

  function bind() {
    $('buy')?.addEventListener('click', e => { e.preventDefault(); void open('LONG'); }, true);
    $('sell')?.addEventListener('click', e => { e.preventDefault(); void open('SHORT'); }, true);
    $('close')?.addEventListener('click', e => { e.preventDefault(); void close('manual close'); }, true);
    $('resetPaper')?.addEventListener('click', e => { e.preventDefault(); void resetPaperAccount(); }, true);
  }

  bind(); render(); void readDurable();
  setInterval(() => { void monitorPosition(); void readDurable(); }, POLL_MS);
